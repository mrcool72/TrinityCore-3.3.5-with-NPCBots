...
inline ObjectGuid AutonomousMakeGuid(uint64 rawGuid)
{
    if (!rawGuid)
        return ObjectGuid::Empty;

    HighGuid const high = static_cast<HighGuid>((rawGuid >> 48) & 0xFFFF);
    uint32 const entry = static_cast<uint32>((rawGuid >> 24) & 0xFFFFFF);
    ObjectGuid::LowType const counter = static_cast<ObjectGuid::LowType>(rawGuid & 0xFFFFFF);

    switch (high)
    {
        case HighGuid::Player:
            return ObjectGuid::Create<HighGuid::Player>(static_cast<ObjectGuid::LowType>(rawGuid & 0xFFFFFFFF));
        case HighGuid::Unit:
            return ObjectGuid::Create<HighGuid::Unit>(entry, counter);
        case HighGuid::Pet:
            return ObjectGuid::Create<HighGuid::Pet>(entry, counter);
        case HighGuid::Vehicle:
            return ObjectGuid::Create<HighGuid::Vehicle>(entry, counter);
        case HighGuid::GameObject:
            return ObjectGuid::Create<HighGuid::GameObject>(entry, counter);
        case HighGuid::DynamicObject:
            return ObjectGuid::Create<HighGuid::DynamicObject>(counter);
        case HighGuid::Corpse:
            return ObjectGuid::Create<HighGuid::Corpse>(counter);
        default:
            return ObjectGuid::Empty;
    }
}
